...
public class OperatorDAO extends C3P0BaseDAO {
	...
	public void add(Operator operator) throws ClassNotFoundException, SQLException {
		...
		pstmt.setString(5,operator.getYuliu());
	...
	public void merge(Operator operator) throws ClassNotFoundException, SQLException {
		...
		pstmt.setString(4,operator.getYuliu());
		...
		while(rs.next()) {
			...
			*/
			...
			operators.add(new Operator(rs.getInt("id"), rs.getString("username"), rs.getString("password"),rs.getString("tel"), 
					rs.getString("pic"),rs.getString("yuliu"),new RoleDAO().findByPrimaryKey(rs.getInt("role"))));
		}
		//返回数组
		return operators;
	}
	...
	public Operator findByPrimaryKey(int id) throws ClassNotFoundException, SQLException {
		...
		if(rs.next()) {
			...
			String yuliu = rs.getString("yuliu"); //yuliu为字段名称
			int roleid = rs.getInt("role"); //role为字段名称,操作员表中的外键
			//根据外键上的角色编号，查找完整角色信息（角色实体类对象 ）
			RoleDAO roleDAO = new RoleDAO();
			Role role = roleDAO.findByPrimaryKey(roleid);
			
			operator = new Operator(operatorid, name,password,tel,pic,yuliu,role); //把数据包装成Operator实体对象
		}
		//返回数组
		return operator;
	}
