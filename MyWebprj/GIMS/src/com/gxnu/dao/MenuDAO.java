...
public class MenuDAO extends C3P0BaseDAO {
	...
	public void add(Menu menu) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement pstmt=
				conn.prepareStatement("insert into menu(name,parent,url,img) values(?,?,?)");
	...
	public void merge(Menu menu) throws ClassNotFoundException, SQLException {
		...
		//此处的？表示“占位符”，下标从1开始递增
		PreparedStatement pstmt=
				conn.prepareStatement("update menu set name=?,parent=?,url=?,img=? where id=?");
	...
	public List<Menu> findAll() throws ClassNotFoundException, SQLException {
		...
		while(rs.next()) {
			...
			*/
			...
			menus.add(new Menu(rs.getInt("id"), rs.getString("name"),rs.getInt("parent"),rs.getString("url"),rs.getString("img")));
		}		
		return menus;	//返回集合
	}
	...
	public Menu findByPrimaryKey(int id) throws ClassNotFoundException, SQLException {
		...
		if(rs.next()) {
			...
			String img = rs.getString("img"); //name为字段名称
			menu = new Menu(menuid,name,parent,url,img); //把数据包装成Menu实体对象
		}
		//返回数组
		return menu;
	}
