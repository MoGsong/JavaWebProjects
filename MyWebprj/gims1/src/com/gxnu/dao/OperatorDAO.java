...
public class OperatorDAO extends C3P0BaseDAO {
	...
	public void add(Operator operator) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement pstmt = 
				conn.prepareStatement("insert into operator(username,password,tel,pic,yuliu,operator) values(?,?,?,?,?,?)");
		//设置点位符的值,注意数据类型的选择
	...
	public void merge(Operator operator) throws ClassNotFoundException, SQLException {
		...
		//此处的？表示“占位符”，下标从1开始递增，须在后续设置其值
		PreparedStatement pstmt = 
				conn.prepareStatement("update operator set password=?,tel=?,pic=?,yuliu=?,operator=? where id=?");
		//设置点位符的值,注意数据类型的选择
	...
	public List<Operator> findAll() throws ClassNotFoundException, SQLException {
		//定义数组
//		Operator[] operators = new Operator[findTotal()];   //默认每个元素均为null
//		int i = 0;
		...
		PreparedStatement pstmt = 								 //3、基于数据库连接对象 java.sql.Connection,创建数据库操作对象java.sql.PreparedStatement，须绑定一个SQL语句
				conn.prepareStatement("select * from operator"); 	 //此处的？表示“占位符”，下标从1开始递增，须在后续设置其值													
		...
		while(rs.next()) {
			...
			Operator operator = new Operator(id, name); //把数据包装成Operator实体对象
...
//			operators[i++] = new Operator(rs.getInt("id"), rs.getString("username"), rs.getString("password"),
//											rs.getString("tel"),rs.getString("pic"),rs.getString("yuliu"),new RoleDAO().findByPrimaryKey(rs.getInt("role")));
			operators.add(new Operator(rs.getInt("id"), rs.getString("username"), rs.getString("password"),
					rs.getString("tel"),rs.getString("pic"),new RoleDAO().findByPrimaryKey(rs.getInt("role"))));
		}
		
		return operators;	//返回数组
	}
	...
	public Operator findByPrimaryKey(int id) throws ClassNotFoundException, SQLException {
		...
		if(rs.next()) {
			...
			operator = new Operator(operatorid, name,password,tel,pic,role); //把数据包装成Operator实体对象
		}
		//返回数组
		return operator;
	}
