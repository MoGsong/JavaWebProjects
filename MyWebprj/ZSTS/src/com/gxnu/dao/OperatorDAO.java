...
public class OperatorDAO extends C3P0BaseDAO {
	 ...
	 */
	/*
	 * public Operator Find(String user,String email) throws ClassNotFoundException,
	 * SQLException { Operator operator = new Operator(); //1、加载驱动 2、连接rbac数据库
	 * Connection conn = super.getConnection(); //3、基于数据库连接对象
	 * java.sql.Connection,创建数据库操作对象java.sql.PreparedStatement，须绑定一个SQL语句
	 * //此处的？表示“占位符”，下标从1开始递增，须在后续设置其值 PreparedStatement pstmt =
	 * conn.prepareStatement("select * from operator where username=? AND email=?");
	 * pstmt.setString(1,user); pstmt.setString(2,email);
	 * //4、基于数据操作对象java.sql.PreparedStatement，创建数据库结果集对象java.sql.ResultSet ResultSet
	 * rs = pstmt.executeQuery();
	 * 
	 * //因为结果集中只有一条记录，无需循环遍历结果集，只需要让指针移动一次即可 if(rs.next()) { //每循环一次，获取指针定位的某条记录上的数据
	 * int operatorid = rs.getInt("id"); //id为字段名称 String username =
	 * rs.getString("username");//name为字段名称 String password =
	 * rs.getString("password"); String tel = rs.getString("tel"); String pic =
	 * rs.getString("pic"); String pre = rs.getString("pre"); String mail =
	 * rs.getString("email"); Role role = new
	 * RoleDao().selectOne(rs.getInt("role"));//因为操作员表关联角色表，所以要额外查询 operator = new
	 * Operator(operatorid,username,password,tel,pic,role,pre); //把数据包装成Operator实体对象
	 * } //返回数组 return operator; }
	 */
