...
<body style="background-color: white">
   <div class="col-lg-12">
			          ...
			          %>
                                        ...
                                        <tr class="gradeA even">
                                            <td class="sorting_1"><%=menus.indexOf(menu)+1%><%="id="+menu.getId() %></td>
                                            <td class=" "><%=menu.getName() %></td>
                                            <td class="center "><%-- <%=menu.getParent() %> --%>
                                            <span> <!-- java中的条件表达式x?y:z   x为true，y为整个表达式的结果，否则z为整个表达式的结果 -->            
           									 <%=new MenuService().find(menu.getParent()).getName()==null?"无":new MenuService().find(menu.getParent()).getName() %> </span></td>
