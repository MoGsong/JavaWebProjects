                        ...
                        <div class="faq-item">
                            <ul class="accordion">
                                ...
                                <li class="wow fadeInUp" data-wow-delay=".3s">
                                    <a>预约医生：<%=new DoctorService().find(registration.getDoctor().getId()).getName() %>&nbsp;&nbsp;预约的时间：<%=df.format(registration.getReserve_time()) %></a>
                                    <p>您的预约编号是：<%=registration.getNumber() %>&nbsp;&nbsp;预约费用是：<%=new DoctorService().find(registration.getDoctor().getId()).getPost().getPrice() %>
                                    &nbsp;&nbsp;支付预约费时间：<%=registration.getPay_time() %></p>
                                    
                                    &nbsp;&nbsp;<a target="IFrameBox" href="<%=Data.URL%>/login?registrationId=<%=registration.getId() %>&patientId=<%=registration.getPatient().getId() %>">进入会诊室</a>
