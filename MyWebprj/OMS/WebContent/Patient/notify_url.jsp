 ...
 */
	...
	Timestamp newT = new Timestamp(nowTime.getTime());
	registration.setPay_time(newT);
	new RegistrationService().add(registration);
	System.out.print("写入测试："+ new RegistrationService().findByDoctorId(registration.getDoctor().getId()));
	...
	ReserveCase reserveCase = reserveCaseService.findByDoctorId(registration.getDoctor().getId());
	//由于患者预约成功，故可预约表中的已预约人数+1，然后更改Registration对象的Pay_time，用于后续修改
	reserveCase.setAl_reserve(reserveCase.getAl_reserve() + 1);
	System.out.println("这是reserveCase:"+ reserveCase.toString());
	//更改Registration的记录
	reserveCaseService.update(reserveCase);
	
	//更改Registration对象的Number，用于后续修改
	registration.setNumber(String.valueOf(reserveCase.getAl_reserve()));
	//输出验证
	System.out.println("这是registration:"+ registration.toString());
