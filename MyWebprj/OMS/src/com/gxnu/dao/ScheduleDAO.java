...
public class ScheduleDAO extends C3P0BaseDAO {
	...
	public void add(Schedule schedule) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement pstmt = conn
				.prepareStatement("insert into schedule (doctor,period,week,day) values(?,?,?,?)");
...
//			Unit unit = unitDAO.findById(unit_id);
	...
	/**
	 * 查询某科室内的同一周天并且在同一时间段值班的医生
	 * 
	 * @return
	 * @throws ClassNotFoundException
	 * @throws SQLException
	 */
	public List<Schedule> findByWeekByPeriodByUnitId(int week, String period,int unitId) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement pstmt = conn.prepareStatement("select * from schedule where week =? and period =?  and doctor in (select id from doctor where unit = ?)");
		pstmt.setInt(1, week);
...
//			Unit unit = unitDAO.findById(unit_id);
	...
	/**
	 * 查询某医生的同一周天并且在同一时间段值班
	 * 
	 * @return
	 * @throws ClassNotFoundException
	 * @throws SQLException
	 */
	public List<Schedule> findByWeekByPeriodByDoctorId(int week, String period,int doctorId) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement pstmt = conn.prepareStatement("select * from schedule where week =? and period =?  and doctor = ?");
		pstmt.setInt(1, week);
		pstmt.setString(2, period);
